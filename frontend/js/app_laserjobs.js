...
function refresh_preview(reload_data, read_passes_widget) {
  ...
  DataHandler.draw_processing_area(preview_canvas_obj, app_settings.to_canvas_scale); // satoshi sakaguchi ADDED
...
function populate_job_queue_divide() {
  $.getJSON("/queue/list/Divide", function(data){
    if (typeof(data.sort) == 'function') {
      data.sort();
    }
    $.each(data, function(index, name) {
      add_to_job_queue_divide(name);
    });
  });
}
...
// satoshi sakaguchi ADDED START
function save_and_add_to_job_queue_divide(name, jobdata) {
  if ((typeof(name) == 'undefined') || ($.trim(name) == '')) {
    $().uxmessage('error', "File name is undefined!");
  }
  //// store jobdata - on success add to queue
  $.post("/queue/save/Divide", { 'job_name':name, 'job_data':jobdata }, function(data) {
    if (data != "1") {
      $().uxmessage('error', "Failed to store G-code. (divide)");
    }
  });
}

function add_to_job_queue_divide(name) {
  //// add list item to page
  $('#job_queue_divide').prepend('<li><a href="#" class="add-canvas"><span>'+ name +'</span><span class="pull-right"></span></a></li>')
  //// action for loading gcode
  $('#job_queue_divide li:first a.add-canvas').click(function(){
    var name = $(this).children('span:first').text();
    $.when(
      $.get("/queue/get/" + name + "/Divide", function(jobdata) {
        load_into_job_widget(name, jobdata);
      }).error(function() {
        $().uxmessage('error', "ファイルが見つかりません: " + name);
      })
    ).done(function() {
      $.get("/queue/rm/" + name + "/Divide", function(data) {
        if (data == "1") {
          $('#job_queue_divide').html('');
          populate_job_queue_divide();
        } else {
          $().uxmessage('error', "failed to clear queue divide.");
        }
      }).error(function(){
        $().uxmessage('error', "削除失敗。ファイルが見つかりません: " + name);
      })
    });
    return false;
  });
