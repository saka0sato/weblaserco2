...
// I:Raster Start
  addRasters : function(rasters_by_color) {
    ...
    for (var color in rasters_by_color) {
      ...
      //if (color == 0)
      //  color = '#0000ff'
      color = '#0000ff'; // 20160213 satoshi sakaguchi DEBUGED: Raster image is one or more. All raster image's color #0000ff
      if (!this.rasters_by_color[color])
        this.rasters_by_color[color] = [];
      if (!this.paths_by_color[color])
        this.paths_by_color[color] = [];
...
// C:Raster End
  ...
  divideArray : function(arr, color, d_num) {
    var idx = 0;
    var results = [];
    var arr_length = arr[color].length;
    var i = 0;

    while (idx + d_num < arr_length) {
      var result = [];
      result[color] = arr[color].slice(idx, idx+d_num);
      results[i] = {};
      results[i][color] = result[color];
      idx = idx + d_num;
      i++;
    }
    if (arr_length > 0) {
      var rest = [];
      rest[color] = arr[color].slice(idx, arr_length+1);
      results[i] = {};
      results[i][color] = rest[color];
    }

    return results;
  },

  getDivideJson : function(exclude_colors) {
    // write internal format
    // exclude_colors is optional
    const d_num_raster = 16;  // divide num for raster
    const d_num_path = 512;    // divide num for path
    var paths_by_color = this.paths_by_color;
// I:Raster Start
    var rasters_by_color = this.rasters_by_color;
// I:Raster End
...
// I:Raster Start
      ...
      for (var color in this.rasters_by_color) {
        if (!(color in exclude_colors)) {
          if (this.rasters_by_color[color].length > 0) {
            rasters_by_color[i] = this.divideArray(this.rasters_by_color, color, d_num_raster);
            i++;
          }
        }
      }
// I:Raster End
      ...
      for (var color in this.paths_by_color) {
        if (!(color in exclude_colors)) {
          if (this.paths_by_color[color].length > 0) {
            paths_by_color[j] = this.divideArray(this.paths_by_color, color, d_num_path);
            j++;
          }
        }
      }
...
// I:Raster Start
        // Rasters
        if (typeof this.rasters_by_color[color] !== 'undefined') {
          var rasters = this.rasters_by_color[color];
...
// C:刻印不具合修正 End
        ...
        // Paths
// I:Raster End
        if (typeof this.paths_by_color[color] !== 'undefined') {
          var paths = this.paths_by_color[color];
      ...
      if (exclude_colors === undefined || !(color in exclude_colors)) {
        ...
        if (rasters) {
          for (var k=0; k<rasters.length; k++) {
            ...
            //var offset = 20;
            var offset = 0; // satoshi sakaguchi
...
// I:Raster End
  ...
  // satoshi sakaguchi ADDED
  draw_processing_area : function (canvas, scale) {
    xmin = 0;
    ymin = 0;
    xmax = parseInt($('#width_mm').val()) * scale;
    ymax = parseInt($('#height_mm').val()) * scale;
