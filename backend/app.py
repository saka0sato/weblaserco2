...
#### satoshi sakaguchi ADDED START
def storage_dir(tmp_dir=None):
    directory = ""
    if sys.platform == 'darwin':
        ...
        if tmp_dir is None:
          ...
          directory = os.path.join(os.path.expanduser('~'), 'Library', 'Application Support', COMPANY_NAME, APPNAME, tmp_dir)
    elif sys.platform == 'win32':
        if tmp_dir is None:
          directory = os.path.join(os.path.expandvars('%APPDATA%'), COMPANY_NAME, APPNAME)
        else:
          directory = os.path.join(os.path.expandvars('%APPDATA%'), COMPANY_NAME, APPNAME, tmp_dir)
    else:
        if tmp_dir is None:
          directory = os.path.join(os.path.expanduser('~'), "." + APPNAME)
        else:
          directory = os.path.join(os.path.expanduser('~'), "." + APPNAME, tmp_dir)

    if not os.path.exists(directory):
        os.makedirs(directory)
        
    return directory
#### satoshi sakaguchi ADDED END
...
@route('/queue/get/:name#.+#/:option#.+#')
def static_queue_handler(name, option=None):
    if option != 'Divide':
      return static_file(name, root=storage_dir(), mimetype='text/plain')
    else:
      return static_file(name, root=storage_dir('Divide'), mimetype='text/plain')

@route('/queue/list')
@route('/queue/list/:option#.+#')
def queue_list_handler(option=None):
    # return a json list of file names
    files = []
    cwd_temp = os.getcwd()
    try:
...
@route('/queue/save/:option#.+#', method='POST')
def queue_save_handler(option=None):
    ret = '0'
    if 'job_name' in request.forms and 'job_data' in request.forms:
        ...
        if option != 'Divide':
          ...
          filename = os.path.abspath(os.path.join(storage_dir('Divide'), name.strip('/\\')))
        ...
        if option != 'Divide':
          ...
          print "error: save failed, invalid POST request (divide)"
    return ret


@route('/queue/dclear')
def queue_dclear_handler():
    # remove files in directory
    for root, dirs, files in os.walk(storage_dir('Divide'), topdown=False):
      for name in files:
...
@route('/queue/rm/:name')
@route('/queue/rm/:name/:option')
def queue_drm_handler(name, option=None):
    ...
    if option != 'Divide':
      ...
      filename = os.path.abspath(os.path.join(storage_dir('Divide'), name.strip('/\\')))
      if filename.startswith(storage_dir('Divide')):
        if os.path.exists(filename):
            try:
                os.remove(filename);
                print "file deleted: " + filename
...
argparser = argparse.ArgumentParser(description='Run WebLaser.', prog='weblaser')
...
argparser.add_argument('-n', '--dry-run', dest='dryrun', action='store_true',
                    default=False, help='use this for dry run')
args = argparser.parse_args()



print "WebLaser " + VERSION
...
if args.list_serial_devices:
    ...
    if not SERIAL_PORT:
        print "-----------------------------------------------------------------------------"
        print "WARNING: WebLaser doesn't know what serial device to connect to!"
    ...
    if args.flash:
        ...
        if args.host_on_all_interfaces:
          ...
          if args.dryrun:
            ...
            run_with_callback(my_hostname, NETWORK_PORT)
