...
# satoshi sakaguchi ADDED matrixApply matrixApply2
from .utilities import matrixMult, matrixApply, matrixApply2, parseFloats 
...
#        log.warn("'image' tag is not supported, ignored")     
        ...
        trans_image_size = list(image.size)
        matrixApply2(node['xformToWorld'], trans_image_size)

        image = image.resize((int(trans_image_size[0]), int(trans_image_size[1])))
        # transform width & height size
        width = trans_image_size[0]
        height = trans_image_size[1]
        # illustrator's svg file is not supported rotate writting 
        ...
        print "RASTER_WIDTH_MAX(" + str(converted_image.size[0]) + "/" + str(RASTER_WIDTH_MAX) + ")"
        print "RASTER_HEIGHT(" + str(converted_image.size[1]) + ")"
        print "raw_x:" + str(x) + " raw_y:" + str(y)
        ...
        #=== satoshi sakaguchi START
            ...
            converted_image_list = self._divideImage(converted_image, x, y, width, height)
            for divided_image in converted_image_list:
                raster = []
                raster_data = divided_image[0].getdata()
                raster.append([divided_image[1], divided_image[2]])
                raster.append([divided_image[3], divided_image[4]])
                raster.append([divided_image[0].size[0], divided_image[0].size[1]])
                raster.append(list(raster_data))
                node['rasters'].append(raster)
        #=== satoshi sakaguchi END
# C:Raster End

    def _divideImage(self, c_image, x, y, width, height):
        #width_division_number = 8 # the number after the division (width)
        #height_division_number = 5 # alternate RASTER_HEIGHT_MAX
        width_division_number = 4 # the number after the division (width)
        height_division_number = 4 # alternate RASTER_HEIGHT_MAX

        r_image_width = c_image.size[0] / width_division_number
        r_image_height = c_image.size[1] / height_division_number

        width = width / width_division_number
        height = height / height_division_number

        image_list = []
        raw_x = x
        raw_y = y
        for i in range(width_division_number):
            image_x_min = raw_x + i * r_image_width
            image_x_max = raw_x + (i+1) * r_image_width
            for j in range(height_division_number):
                image_y_min = raw_y + j * r_image_height
                image_y_max = raw_y + (j+1) * r_image_height
                d_image = c_image.crop((image_x_min, image_y_min, image_x_max, image_y_max))
                #print d_image.size
                x = raw_x + i * width
                y = raw_y + j * height
                image_list.append([d_image, x, y, width, height])
        return image_list
